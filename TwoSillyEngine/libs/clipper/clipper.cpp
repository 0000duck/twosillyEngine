...
struct TEdge {
  ...
  PolyType PolyTyp;
  EdgeSide Side;
  int WindDelta; //1 or -1 depending on winding direction 1或-1取决于绕组的方向windcnt int；
  int WindCnt;
  int WindCnt2; //winding count of the opposite polytype对多绕组数
  int OutIdx;
  TEdge *Next;
  TEdge *Prev;
  TEdge *NextInLML;
...
bool ClipperBase::AddPath(const Path &pg, PolyType PolyTyp, bool Closed)
{
  ...
  //传进来的链表的长度-1（链表最后一个元素的下标）
  int highI = (int)pg.size() -1;

  //如果是闭合的链表就把最后一个最后一个下标指针在往前移一格
